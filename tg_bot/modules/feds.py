...
def fed_ban(bot: Bot, update: Update, args: List[str]):
    ...
    if fban:
        ...
        if not x:
            message.reply_text(
                "Failed to ban from the federation! If this problem continues, ask @thedenomparkour for help!"
            )
            return
    ...
    if not x:
        message.reply_text(
            "Failed to ban from the federation! If this problem continues, ask i@thedenomparkour for help."
        )
        return
...
def set_frules(bot: Bot, update: Update, args: List[str]):
    ...
    if len(args) >= 1:
        ...
        if not x:
            update.effective_message.reply_text(
                "Big F! There is an error while setting federation rules! If you wonder why, please ask @thedenomparkour!"
            )
            return
